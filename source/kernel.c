					  ...
					  float Cap_1, float Rx_1, float Ry_1, float Rz_1, float amb_temp)
{
	...
	{
		float teste1 =temp[r*col+c]+ ( Cap_1 * (power[r*col+c] + (temp[(r+1)*col+c] + temp[(r-1)*col+c] - 2.f*temp[r*col+c]) * Ry_1 + (temp[r*col+c+1] + temp[r*col+c-1] - 2.f*temp[r*col+c]) * Rx_1 + (amb_temp - temp[r*col+c]) * Rz_1));
		printf("index: %d\n", r*col+c);
